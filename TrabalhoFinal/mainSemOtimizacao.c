...
void init(int iteracoes, int linhas, int colunas, double* u) {
    ...
    for (int k = 0; k < iteracoes - 1; k++) {
        for (int i = 1; i < linhas - 1; i++) {
            for (int j = 1; j < colunas - 1; j++) {
                ...
                u[pos(k+1, i, j, linhas, colunas)] = gamma *    (u[pos(k, i+1, j, linhas, colunas)] +
                                                                u[pos(k, i-1, j, linhas, colunas)] +
                                                                u[pos(k, i, j+1, linhas, colunas)] +
                                                                u[pos(k, i, j-1, linhas, colunas)] -
                                                                4 * u[pos(k, i, j, linhas, colunas)]) +
                                                                u[pos(k, i, j, linhas, colunas)];
            }
        }
    }
}
...
double* generate_matrix_seq(int iteracoes, int linhas, int colunas, double coeficiente) {
    ...
    double *u = (double*) malloc(sizeof(double) * iteracoes * linhas * colunas);
    if (!u) {
    ...
    init(iteracoes, linhas, colunas, u);
    ...
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            fprintf(file, "%.6lf ", u[pos(iteracoes-1, i, j, linhas, colunas)]);
        }
        fprintf(file, "\n");
    }
...
double* generate_matrix_conc(int iteracoes, int linhas, int colunas, double coeficiente, int num_threads){
    ...
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            fprintf(file, "%lf ", u[pos(iteracoes-1, i, j, linhas, colunas)]);
        }
        fprintf(file, "\n");
    }
...
double comp_matrices(double *u1, double *u2, int iteracoes, int linhas, int colunas){
    for(int i=0; i<linhas; i++){
        for(int j=0; j<colunas; j++){
            u1[pos(iteracoes-1, i, j, linhas, colunas)] -= u2[pos(iteracoes-1, i, j, linhas, colunas)];
        }
    }
    ...
    for(int i=0; i<linhas; i++){
        for(int j=0; j<colunas; j++){
            soma += u1[pos(iteracoes-1, i, j, linhas, colunas)] * u1[pos(iteracoes-1, i, j, linhas, colunas)];
        }
    }
